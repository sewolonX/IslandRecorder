package com.island.recorder.core.reflection
...
class ReflectionProviderImpl : ReflectionProvider {
    ...
    override fun getDeclaredConstructor(
        ...
        val key = "decl:" + clazz.name + parameterTypes.joinToString(
            prefix = "(",
            postfix = ")"
        ) { it.name }
        return constructorCache.getOrPut(key) {
    ...
    override fun getMethod(
        ...
        val key = clazz.name + "#" + name + parameterTypes.joinToString(
            prefix = "(",
            postfix = ")"
        ) { it.name }
    ...
    override fun getDeclaredMethod(
        ...
        val key = "decl:" + clazz.name + "#" + name + parameterTypes.joinToString(
            prefix = "(",
            postfix = ")"
        ) { it.name }
        return methodCache.getOrPut(key) {
    ...
    private inline fun <K : Any, V : Any> ConcurrentHashMap<K, V>.getOrPut(
        key: K,
        defaultValue: () -> V?
    ): V? {
