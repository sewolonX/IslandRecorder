package com.island.recorder.util
...
class NotificationHelper(
    ...
    fun createRecordingNotification(
        ...
        // Add HyperOS Focus Island (超级�? params
    ...
    private fun addFocusIslandParams(
        ...
        val action1IconDarkKey =
            if (isPaused) "miui.focus.pic_resume_dark" else "miui.focus.pic_pause_dark"
        ...
        val picsBundle = Bundle().apply {
            ...
            // Dark mode icons (white, for 超级�?dark background)
            putParcelable(
    ...
    fun updateNotification(notification: Notification, bypass: Boolean = false) {
        if (bypass && privilegedOperations.capability().hasPrivilegedOperations) {
            ...
            // Sync block
            val blocked = privilegedOperations.setPackageNetworkingEnabled(xmsfUid, false)
            notificationManager.notify(NOTIFICATION_ID, notification)
            ...
            if (blocked) {
                Thread {
                    ...
                    privilegedOperations.setPackageNetworkingEnabled(xmsfUid, true)
                }.start()
            }
        } else {
            notificationManager.notify(NOTIFICATION_ID, notification)
